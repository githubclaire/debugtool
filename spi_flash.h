...
#define SF_CONTROL         0x8a30  //30;31;32;33
...
typedef union 
{
	...
	struct 
	{
		unsigned int reserved0     :24; //reserved[23:0];
             unsigned int nop0		:2;    
		unsigned int data_len	:3;	// data length:0-4 bytes
		unsigned int rw		:1; // 0:read, 1:write
		unsigned int addr_en	:1; // 0: not send 3 bytes address, 1: send 3 bytes address.
		unsigned int en_flag	:1; // write: write one to start once transform; read: 0: free, 1: busy
	}reg;	
}Reg_SF_Control;
